#!/usr/bin/env python3
"""Verify Missouri rate changes period-over-period."""
...
# Check MO rate periods
print("=" * 60)
print("MISSOURI RATE PERIODS")
print("=" * 60)
...
print("\n" + "=" * 60)
print("MISSOURI RATE CHANGES (Period-over-Period)")
print("=" * 60)
...
cursor.execute("""
    ...
    FROM v_rate_changes
    WHERE state = 'MO' AND prior_rate IS NOT NULL
""")
...
cursor.execute("""
    ...
    FROM medicaid_rates
    WHERE state = 'MO' AND end_date IS NULL
""")
summary = cursor.fetchone()
print(f"Current MO rates: {summary['total']}")
print(f"Matched to property_master: {summary['matched']} ({summary['match_pct']}%)")
print("\nNote: MO HealthNet SFY 2026 rates (7/1/2025 - 6/30/2026)")
