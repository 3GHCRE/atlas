#!/usr/bin/env python3
"""Load Indiana Medicaid rates into database."""
import csv
...
# Read CSV
facilities = []
with open('IN_2025-07_rates.csv', 'r', encoding='utf-8') as f:
...
# Check existing IN rates
cursor.execute("""
    SELECT COUNT(*) as cnt, MAX(effective_date) as latest
    FROM medicaid_rates
    WHERE state = 'IN' AND end_date IS NULL
""")
existing = cursor.fetchone()
print(f"Existing current IN rates: {existing['cnt']} (latest: {existing['latest']})")
...
for f in facilities:
    cursor.execute(insert_sql, (
        ...
        f['state_facility_id'],
        float(f['daily_rate']),
        'case_mix',  # Indiana uses case-mix (blended) rates
        effective_date,
        f['source_file'],
...
# Summary
print("\n" + "=" * 50)
print("INDIANA RATE UPDATE COMPLETE")
print("=" * 50)
print(f"  Prior records closed: {closed}")
print(f"  New records inserted: {inserted}")
print(f"  Effective date: {effective_date}")
print("  Note: Rates are Blended Case Mix rates from Prospective system")
