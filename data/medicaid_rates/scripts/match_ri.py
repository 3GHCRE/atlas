#!/usr/bin/env python3
"""Match Rhode Island Medicaid rates to property_master using fuzzy matching."""
...
# Get unmatched RI rates
cursor.execute("""
    SELECT id, facility_name, state_facility_id
    FROM medicaid_rates
    WHERE state = 'RI' AND property_master_id IS NULL AND end_date IS NULL
""")
unmatched = cursor.fetchall()
print(f"Unmatched RI rates: {len(unmatched)}")

# Get RI facilities from property_master
cursor.execute("""
...
print(f"RI facilities in property_master: {len(pm_facilities)}")
