#!/usr/bin/env python3
"""Match Virginia Medicaid rates to property_master using fuzzy matching."""
...
# Get unmatched VA rates
cursor.execute("""
    SELECT id, facility_name, state_facility_id
    FROM medicaid_rates
    WHERE state = 'VA' AND property_master_id IS NULL AND end_date IS NULL
""")
unmatched = cursor.fetchall()
print(f"Unmatched VA rates: {len(unmatched)}")

# Get VA facilities from property_master
cursor.execute("""
...
print(f"VA facilities in property_master: {len(pm_facilities)}")
