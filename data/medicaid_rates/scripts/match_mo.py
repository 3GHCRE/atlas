#!/usr/bin/env python3
"""Match Missouri Medicaid rates to property_master using fuzzy matching."""
...
# Get unmatched MO rates
cursor.execute("""
    SELECT id, facility_name, state_facility_id
    FROM medicaid_rates
    WHERE state = 'MO' AND property_master_id IS NULL AND end_date IS NULL
""")
unmatched = cursor.fetchall()
print(f"Unmatched MO rates: {len(unmatched)}")

# Get MO facilities from property_master
cursor.execute("""
...
print(f"MO facilities in property_master: {len(pm_facilities)}")
