#!/usr/bin/env python3
"""Verify Indiana rate changes period-over-period."""
...
# Check IN rate periods
print("=" * 60)
print("INDIANA RATE PERIODS")
print("=" * 60)
...
print("\n" + "=" * 60)
print("INDIANA RATE CHANGES (Period-over-Period)")
print("=" * 60)
...
cursor.execute("""
    ...
    FROM v_rate_changes
    WHERE state = 'IN' AND prior_rate IS NOT NULL
""")
...
print(f"Current IN rates: {summary['total']}")
print(f"Matched to property_master: {summary['matched']} ({summary['match_pct']}%)")
print("\nNote: Indiana uses Blended Case Mix Rate from Prospective system.")
