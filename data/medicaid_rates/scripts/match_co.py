#!/usr/bin/env python3
"""Match Colorado Medicaid rates to property_master using fuzzy matching."""
...
# Get unmatched CO rates
cursor.execute("""
    SELECT id, facility_name, state_facility_id
    FROM medicaid_rates
    WHERE state = 'CO' AND property_master_id IS NULL AND end_date IS NULL
""")
unmatched = cursor.fetchall()
print(f"Unmatched CO rates: {len(unmatched)}")

# Get CO facilities from property_master
cursor.execute("""
...
print(f"CO facilities in property_master: {len(pm_facilities)}")
